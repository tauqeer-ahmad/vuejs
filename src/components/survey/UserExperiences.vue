<template>
  <section>
    <base-card>
      ...
      <p v-else-if="!isLoading && (!results || results.length === 0)">No Experiences Found! Add some ...</p>
...
export default {
  ...
  methods: {
    loadExperiences() {
      this.isLoading = true;
      fetch('https://vuejs-demo-http-8daaa-default-rtdb.firebaseio.com/.json').then((response) => {
        if (response.ok) {
